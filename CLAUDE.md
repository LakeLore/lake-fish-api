...
- Entitlement gate via `entitlement.js` middleware: paid-state `/lake/:id` and `/pdf` return 402 without entitlement. Paid-state `/results` passes through in **preview mode** for non-subscribers (`req.lakeLorePreview = true`): the canonical results handler returns all metrics but sets `lake_name: null` on every row and adds `preview: true` to the response — powers the in-app paid-state preview (blurred names, paywall on lake taps). `/status` and `/filters` stay public. MN is the free state.
