...
| `GET /api/:state/filters` | Available species, gear types (+ `gearTypeCounts`, `gearCpueCounts` — per-gear CPUE-bearing row counts driving the app's default-gear pick — and `gearLatestCounts`, 2026-08-11: distinct-lake counts per gear = what a single-gear query returns under `mostRecentOnly`, shown by the Filters modal when "Latest only" is on), counties, year range | Responses cached in-memory per (path+query), LRU 150 / 6 h TTL, dropped per state on `/reload` (2026-08-25 — the unfiltered MN call cost ~1 s of synchronous CPU per new-user session). |
| `GET /api/:state/measures` | **Measure × Gear/Source manifest** (DATA_MODEL_PROPOSAL_2026-07-20) | Returns the measures with data in the current `species`/`county` scope, in cascade order (Abundance → Stocking Impact → Avg Size → Presence). Each measure nests its **Gear/Source** options (the required filter under Abundance & Avg Size), each source carrying the exact `{gear, cpueKind, sort, stockingFirst, presenceUnion}` the app sends to `/results`, plus `records`/`lakes` coverage and `defaultSourceId` (**most records among quantitative sources** — `sort:'cpue'` measured/relative/normalized rates outrank ranking sources; ratings lead only where no quantitative source exists. Changed 2026-08-14 from plain most-records, which let WI's 1,147-lake coarse Forecast Rating hide its 330-row measured corpus; mirrors the legacy client defaultGearFor rule. Only wi/il/ga manifests flipped). Abundance sources: **one per distinct `gear_category`** — gear/creel rows (`expression:catch-per-unit`) and relative rows (`ranking`) each get their own source scoped by `?gear=` (a LIFA 0–5 rating, % composition, and a gill-net index are not one comparable thing, so they are never merged — 2026-07-25); the genuine `normalized` metric stays ONE merged cross-gear source scoped by `?cpueKind=normalized`; plus forecast rating (one per rating system). Presence is the **derived union** of every lake+species (fish_catch ∪ lake_stocking_metrics). Powers the app's Measure selector + Gear/Source filter. **Trophy Abundance (2026-09-10, schema v8; SINGLE TIER since 2026-09-11 — owner call "only true trophies"):** measure id `trophy` (after Avg Size, never a default) for states with `cpue_memorable` data on the wire (nd/va + held ks; SD has none — its only clean derivation was preferred-plus, below the trophy bar) — one source per gear (`trophy:<gear>`, sort `cpue_memorable` = catch rate of fish ≥ the Gabelhouse MEMORABLE length, e.g. 25" walleye; defaultSourceId prefers non-ranking). `cpue_preferred` stays on the wire, unsurfaced. Additive/new route — `/filters` and `/results` goldens untouched. Cached like `/filters` (2026-08-25). |
...
| `GET /api/:state/lake/:id` | Lake detail with surveys, catches, stocking, computed metrics | Paid states without entitlement get **preview mode** too (2026-07-15): full detail with the same identity fields + report ids/source links redacted, ids hashed; accepts ONLY hashed preview ids from `/results` — a RAW id in preview mode returns `402 subscription_required` (2026-08-25: the public `/lakes-index` carries real ids for SEO pages, so raw-id preview responses let an unauthenticated caller join identity to the full redacted metrics and scrape the paid dataset; app preview flows only ever hold hashed ids). Since 2026-07-17 the ratings-tier states (ga/mo/il/fl/ky/ok/ks) carry `rating`/`rating_ordinal` on the catches list (registry `wire.lakeCatches`) so the detail screen can show their headline metric. |
...
| `GET /api/:state/lakes-index` | Public SEO index (built for the marketing site's per-lake pages, which were REMOVED 2026-09-12 — endpoint now has no consumer; removal candidate) | Lake names + per-lake survey/species/stocking **counts** only, no metrics — powers lakeloreapp.com's programmatic per-lake pages. Deliberately NOT in the entitlement gate (the lake name is the search term; the numbers stay behind the sub). 6-hour in-memory cache per state. |
